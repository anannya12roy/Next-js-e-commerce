...
export default function FeaturesPage({ params }: { params: Promise<{ type: string }> }) {
  ...
  const fetchFeatures = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/features/${type}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId 
        ? `http://localhost:5000/api/features/${type}/${editingId}`
        : `http://localhost:5000/api/features/${type}`;
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure you want to delete this item?')) {
      try {
        const res = await fetch(`http://localhost:5000/api/features/${type}/${id}`, { method: 'DELETE' });
        if (res.ok) {
