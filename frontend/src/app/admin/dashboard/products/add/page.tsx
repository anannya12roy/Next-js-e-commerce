    ...
    const fetchData = async () => {
      try {
        const [catRes, brandRes, attrRes, fitRes, fabRes, embRes, sleeveRes] = await Promise.all([
          ...
          fetch('http://localhost:5000/api/features/fit'),
          fetch('http://localhost:5000/api/features/fabrication'),
          fetch('http://localhost:5000/api/features/embellishment'),
          fetch('http://localhost:5000/api/features/sleeve-length')
        ]);
        ...
        const sleeveData = await sleeveRes.json();

        setCategories(Array.isArray(catData) ? catData : catData.value || []);
        setBrands(Array.isArray(brandData) ? brandData : brandData.value || []);
        setAttributes(Array.isArray(attrData) ? attrData : attrData.value || []);
        setFits(Array.isArray(fitData) ? fitData : fitData.value || []);
        setFabrications(Array.isArray(fabData) ? fabData : fabData.value || []);
        setEmbellishments(Array.isArray(embData) ? embData : embData.value || []);
        setSleeveLengths(Array.isArray(sleeveData) ? sleeveData : sleeveData.value || []);
      } catch (e) {
  ...
  return (
    <AccordionContext.Provider value={{ expanded, toggleSection }}>
        ...
        <div className={styles.mainCol}>
          ...
          <Accordion title="Product Information" sectionKey="info">
            ...
            <div className={styles.formGroup}>
              ...
              <input type="text" className={styles.input} placeholder="Product Name" />
            </div>
      ...
      <div className={styles.footer}>
        <button className={styles.actionButton}>Save Product</button>
      </div>
